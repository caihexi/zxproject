...
class ContentController extends BaseController {
    ...
    def edit(Long id) {
		if (!checkRole(session.user,'content')) {
			flash.message = message(code: 'default.role.message', args: ["edit content[${id}]"])
    ...
    def update(Long id, Long version) {
		if (!checkRole(session.user,'content')) {
			flash.message = message(code: 'default.role.message', args: ["update content[${id}]"])
			redirect(action: "myerror")
			return
		}
        def contentInstance = Content.get(id)
    ...
    def delete(Long id) {
 		if (!checkRole(session.user,'content')) {
			flash.message = message(code: 'default.role.message', args: ["delete content[${id}]"])
			redirect(action: "myerror")
			return
		}
       def contentInstance = Content.get(id)
